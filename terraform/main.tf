...
resource "aws_iam_role" "workspace_runner" {
...
resource "aws_iam_role_policy" "workspace_runner" {
  name = "bhannafi-fargate-test-runner"
  role = "${aws_iam_role.workspace_runner.id}"
  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
resource "aws_ecs_task_definition" "monitor" {
  ...
  task_role_arn = "${aws_iam_role.workspace_runner.arn}"
...
[
  {
    "family": "bhannafi-fargate-test",
    "name": "bhannafi-fargate-test",
    "image": "${var.DCP_WORKSPACE_IMAGE_NAME}",
    "cpu": 256,
    "memory": 512,
    "essential": true,
    "portMappings": [
